from ai9414.core.server import create_fastapi_app
from ai9414.graph_bfs import GraphBfsDemo
from ai9414.graph_bfs.generator import generate_spatial_graph
from ai9414.graph_bfs.solver import solve_graph_bfs
from ai9414.graph_bfs.student import (
    GRAPH_BFS_TRACE_ACTIONS,
    build_unimplemented_graph_bfs_result,
    get_neighbours,
    validate_graph_bfs_solver_result,
    validate_graph_payload,
)
from ai9414.search import run_graph_bfs_solver


def test_graph_bfs_demo_exposes_configurations():
    app = GraphBfsDemo()
    assert app.list_examples() == ["small", "large"]


def test_graph_bfs_trace_reaches_found_state():
    app = GraphBfsDemo()
    trace = app.get_trace_payload()
    assert trace["app_type"] == "graph_bfs"
    assert trace["steps"][0]["event_type"] == "start"
    assert trace["steps"][-1]["event_type"] == "found"
    assert trace["summary"]["result"] == "found"


def test_generate_graph_command_returns_playback_trace():
    app = GraphBfsDemo()
    payload = app.handle_action(
        {
            "action": "app_command",
            "payload": {"command": "generate_graph", "size": "large", "seed": 41},
        }
    )
    assert payload["ok"] is True
    assert payload["state"]["data"]["graph"]["size"] == "large"
    assert payload["state"]["data"]["graph"]["seed"] == 41
    assert payload["trace"]["summary"]["step_count"] > 0
    assert payload["trace"]["summary"]["result"] == "found"


def test_generated_graphs_are_solvable_with_bfs():
    graph = generate_spatial_graph(size="small", seed=17)
    result = solve_graph_bfs(graph)
    assert result.status == "found"
    assert result.path[0] == graph.start
    assert result.path[-1] == graph.goal


def test_graph_bfs_student_helpers_expose_problem_formalisation():
    graph = generate_spatial_graph(size="small", seed=17)
    payload = validate_graph_payload(graph.model_dump())
    neighbours = get_neighbours(payload, payload["start"])
    placeholder = build_unimplemented_graph_bfs_result()
    assert GRAPH_BFS_TRACE_ACTIONS == ("start", "expand", "found", "fail")
    assert isinstance(neighbours, list)
    assert placeholder["status"] == "error"
    assert run_graph_bfs_solver


def test_graph_bfs_result_validation_accepts_canonical_shape():
    result = validate_graph_bfs_solver_result(
        {
            "algorithm": "bfs",
            "status": "found",
            "trace": [
                {
                    "step": 0,
                    "action": "start",
                    "node": "A",
                    "parent": None,
                    "depth": 0,
                    "route": ["A"],
                }
            ],
            "path": ["A"],
            "visited_order": ["A"],
        }
    )
    assert result["status"] == "found"


def test_graph_bfs_fastapi_routes_work():
    app = GraphBfsDemo()
    from fastapi.testclient import TestClient

    with TestClient(create_fastapi_app(app)) as client:
        response = client.get("/api/manifest")
        assert response.status_code == 200
        assert response.json()["app_type"] == "graph_bfs"

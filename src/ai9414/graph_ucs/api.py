"""Student-facing API for the spatial graph UCS demo."""
...
from ai9414.graph_ucs.examples import build_examples
from ai9414.graph_ucs.generator import SIZE_NODE_COUNTS, generate_weighted_graph
from ai9414.graph_ucs.models import GraphUcsConfigModel, GraphUcsExample
from ai9414.graph_ucs.solver import solve_graph_ucs
from ai9414.graph_ucs.trace import build_graph_ucs_trace, build_graph_ucs_trace_from_definition
from ai9414.search.models import WeightedGraph


class GraphUcsDemo(BaseEducationalApp):
    """Playback-first spatial graph UCS demo with optional live Python solving."""

    _MIN_EXPANDED_BY_SIZE = {
        "small": 4,
        "large": 8,
    }
    ...
    def __init__(self, *, mode: str = "student", execution_mode: str = "precomputed") -> None:
        if execution_mode != "precomputed":
            raise AI9414Error(
                ...
                message="GraphUcsDemo currently supports precomputed execution mode only.",
            )
        super().__init__(
            app_type="graph_ucs",
            app_title="Search Demo - Graph UCS",
            execution_mode="precomputed",
            mode=mode,
        )
        self.examples = build_examples()
        self.example: GraphUcsExample | None = self.examples["small"]
    ...
    def load_config(self, path: str) -> None:
        ...
        config = GraphUcsConfigModel.model_validate(raw)
    ...
    def _build_bundle(self):
        if self.example is None:
            return build_graph_ucs_trace_from_definition(
                self.graph,
    ...
    def _generate(self, *, size: str, seed: int | None) -> tuple[WeightedGraph, int]:
        ...
        for _ in range(200):
            ...
            result = solve_graph_ucs(candidate)

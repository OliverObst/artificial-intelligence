"""Trace construction for the spatial graph UCS demo."""
...
from ai9414.graph_ucs.solver import ALGORITHM_LABEL, ALGORITHM_NOTE, solve_graph_ucs
...
def build_blank_graph_ucs_bundle(graph: WeightedGraph) -> TraceBundle:
    initial_state = {
        "example_title": "Generated graph",
        "example_subtitle": "Generate a graph, then use Solve with Python to run uniform-cost search on the same graph.",
...
def build_graph_ucs_trace_from_definition(
    graph: WeightedGraph,
    *,
    title: str,
    subtitle: str,
...
def build_graph_ucs_trace(example: GraphUcsExample) -> TraceBundle:

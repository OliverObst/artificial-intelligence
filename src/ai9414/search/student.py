"""Student-facing helpers for the weighted graph live-Python workflow."""
...
SEARCH_TRACE_ACTIONS: tuple[str, ...] = (
    "start",
    "expand",
    "consider_edge",
    "descend",
...
def validate_weighted_graph_payload(graph: dict[str, Any]) -> dict[str, Any]:
...
def get_neighbours(graph: dict[str, Any] | WeightedGraph, node_id: str) -> list[tuple[str, float]]:
    """Return neighbouring node ids and edge costs in deterministic search order."""
...
def build_unimplemented_weighted_graph_result(message: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "message": message
        or (
            "Implement solve_dfbb in solve_weighted_graph.py. Return algorithm, status, "
...
def validate_weighted_graph_solver_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if result["algorithm"] != "dfbb":
        raise ValueError("Field 'algorithm' must be 'dfbb'.")
    ...
    for expected_step, step in enumerate(trace):
        ...
        if action not in SEARCH_TRACE_ACTIONS:
            raise ValueError(
                "Each trace action must be one of: "
                + ", ".join(SEARCH_TRACE_ACTIONS)
                + "."
            )
...
def run_weighted_graph_solver(
    solver_fn: Callable[[dict[str, Any]], dict[str, Any]],
    *,
    port: int = 9414,
    debug: bool = True,
) -> None:
    """Run the minimal local `/solve` server used by the weighted graph demo."""

    app = FastAPI(title="ai9414 weighted graph solver")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    async def solve(payload: dict[str, Any]) -> JSONResponse:
        ...
        try:
            validated_graph = validate_weighted_graph_payload(graph)
        ...
        try:
            validated_result = validate_weighted_graph_solver_result(result)

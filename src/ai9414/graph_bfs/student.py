"""Student-facing helpers for the spatial graph BFS live-Python workflow."""
...
GRAPH_BFS_TRACE_ACTIONS: tuple[str, ...] = (
    "start",
    "expand",
    "found",
    "fail",
...
def get_neighbours(graph: dict[str, Any] | SpatialGraphDefinition, node_id: str) -> list[str]:
    """Return neighbouring node ids in deterministic BFS order."""
...
def build_unimplemented_graph_bfs_result(message: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "message": message
        or (
            "Implement solve_bfs in solve_graph.py. Return algorithm, status, "
...
def validate_graph_bfs_solver_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if result["algorithm"] != "bfs":
        raise ValueError("Field 'algorithm' must be 'bfs'.")
    ...
    for expected_step, step in enumerate(trace):
        ...
        if action not in GRAPH_BFS_TRACE_ACTIONS:
            raise ValueError(
                "Each trace action must be one of: "
                + ", ".join(GRAPH_BFS_TRACE_ACTIONS)
                + "."
            )
        ...
        if not isinstance(route, list) or not all(isinstance(item, str) for item in route):
            raise ValueError("Field 'route' in each trace entry must be a list of node ids.")
...
def run_graph_bfs_solver(
    solver_fn: Callable[[dict[str, Any]], dict[str, Any]],
    *,
    port: int = 9414,
    debug: bool = True,
) -> None:
    """Run the minimal local `/solve` server used by the live graph BFS demo."""

    app = FastAPI(title="ai9414 spatial graph BFS solver")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    async def solve(payload: dict[str, Any]) -> JSONResponse:
        ...
        try:
            validated_result = validate_graph_bfs_solver_result(result)

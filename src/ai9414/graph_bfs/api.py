"""Student-facing API for the spatial graph BFS demo."""
...
from ai9414.graph_bfs.examples import build_examples
from ai9414.graph_bfs.generator import SIZE_NODE_COUNTS, generate_spatial_graph
from ai9414.graph_bfs.models import GraphBfsConfigModel, GraphBfsExample
from ai9414.graph_bfs.trace import build_graph_bfs_trace, build_graph_bfs_trace_from_definition
from ai9414.graph_dfs.models import SpatialGraphDefinition


class GraphBfsDemo(BaseEducationalApp):
    """Playback-first spatial graph BFS demo with optional live Python solving."""

    def __init__(self, *, mode: str = "student", execution_mode: str = "precomputed") -> None:
        if execution_mode != "precomputed":
            raise AI9414Error(
                code="unsupported_action",
                message="GraphBfsDemo currently supports precomputed execution mode only.",
            )
        super().__init__(
            app_type="graph_bfs",
            app_title="Search Demo - Graph BFS",
            execution_mode="precomputed",
            mode=mode,
        )
        self.examples = build_examples()
        self.example: GraphBfsExample | None = self.examples["small"]
    ...
    def load_config(self, path: str) -> None:
        ...
        config = GraphBfsConfigModel.model_validate(raw)
    ...
    def _generate(
        ...
        *,
        ...
        chosen_seed = random.randint(1, 999_999)
        return generate_spatial_graph(size=size, seed=chosen_seed), chosen_seed

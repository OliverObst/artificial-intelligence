"""Student-facing helpers for the spatial graph greedy best-first live-Python workflow."""
...
GRAPH_GBFS_TRACE_ACTIONS: tuple[str, ...] = (
    "start",
    "expand",
    "consider_edge",
    "enqueue",
...
def get_neighbours(graph: dict[str, Any] | WeightedGraph, node_id: str) -> list[tuple[str, float]]:
    """Return neighbouring node ids and edge costs in deterministic order."""
...
def build_unimplemented_graph_gbfs_result(message: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "message": message
        or (
            "Implement solve_gbfs in solve_graph.py. Return algorithm, status, "
            "trace, path, path_cost, and visited_order."
        ),
        "trace": [],
        "path": [],
        "path_cost": None,
...
def validate_graph_gbfs_solver_result(result: dict[str, Any]) -> dict[str, Any]:
    """Validate the solver result shape returned to the browser."""

    if not isinstance(result, dict):
        raise ValueError("Solver result must be a dictionary.")

    required = {"algorithm", "status", "trace", "path", "path_cost", "visited_order"}
    missing = sorted(required - set(result))
    if missing:
        raise ValueError(f"Solver result is missing required key(s): {', '.join(missing)}.")

    if result["algorithm"] != "gbfs":
        raise ValueError("Field 'algorithm' must be 'gbfs'.")
    ...
    for expected_step, step in enumerate(trace):
        ...
        if action not in GRAPH_GBFS_TRACE_ACTIONS:
            raise ValueError(
                "Each trace action must be one of: " + ", ".join(GRAPH_GBFS_TRACE_ACTIONS) + "."
            )
    ...
    if path_cost is not None and not isinstance(path_cost, (int, float)):
        raise ValueError("Field 'path_cost' must be numeric or null.")
    if result["status"] == "found":
        if not path:
            raise ValueError("A result with status 'found' must include a non-empty path.")
        if path_cost is None:
            raise ValueError("A result with status 'found' must include 'path_cost'.")
    ...
    validated_result = {
        ...
        "path": list(path),
        "path_cost": None if path_cost is None else float(path_cost),
...
def run_graph_gbfs_solver(
    solver_fn: Callable[[dict[str, Any]], dict[str, Any]],
    *,
    port: int = 9414,
    debug: bool = True,
) -> None:
    """Run the minimal local `/solve` server used by the live graph GBFS demo."""

    app = FastAPI(title="ai9414 spatial graph greedy best-first solver")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    async def solve(payload: dict[str, Any]) -> JSONResponse:
        ...
        try:
            validated_result = validate_graph_gbfs_solver_result(result)

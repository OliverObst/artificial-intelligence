"""Student-facing API for the spatial graph greedy best-first demo."""
...
from ai9414.graph_gbfs.examples import build_examples
from ai9414.graph_gbfs.generator import SIZE_NODE_COUNTS, generate_weighted_graph
from ai9414.graph_gbfs.models import GraphGbfsConfigModel, GraphGbfsExample
from ai9414.graph_gbfs.solver import solve_graph_gbfs
from ai9414.graph_gbfs.trace import build_graph_gbfs_trace, build_graph_gbfs_trace_from_definition
from ai9414.search.models import WeightedGraph


class GraphGbfsDemo(BaseEducationalApp):
    """Playback-first spatial graph greedy best-first demo with optional live Python solving."""
    ...
    def __init__(self, *, mode: str = "student", execution_mode: str = "precomputed") -> None:
        if execution_mode != "precomputed":
            raise AI9414Error(
                ...
                message="GraphGbfsDemo currently supports precomputed execution mode only.",
            )
        super().__init__(
            app_type="graph_gbfs",
            app_title="Search Demo - Graph Greedy Best-First",
            execution_mode="precomputed",
            mode=mode,
        )
        self.examples = build_examples()
        self.example: GraphGbfsExample | None = self.examples["small"]
    ...
    def load_config(self, path: str) -> None:
        ...
        config = GraphGbfsConfigModel.model_validate(raw)
    ...
    def _generate(self, *, size: str, seed: int | None) -> tuple[WeightedGraph, int]:
        ...
        for _ in range(200):
            ...
            result = solve_graph_gbfs(candidate)
